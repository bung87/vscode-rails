...
export function getConcernsFilePath(lineStartToWord, fileT: FileType) {
  ...
  let sub = seq.slice(0, -1).join(path.sep),
...
export function controllerDefinitionLocation(
  ...
  if (PATTERNS.CLASS_INHERIT_DECLARATION.test(lineStartToWord)) {
    ...
    let sameModuleControllerSub = path.dirname(
        vscode.workspace
          .asRelativePath(document.fileName)
          .substring(REL_CONTROLLERS.length + 1)
      ),
...
export function getSymbolPath(
  ...
  let [currentClassRaw, parentClassRaw] = line.split("<"),
...
export function getParentControllerFilePathByDocument(
  ...
  let relPath = vscode.workspace.asRelativePath(entryDocument.fileName),
    filePath = getSymbolPath(relPath, line, FileType.Controller);
  return filePath;
}

export function getFunctionOrClassInfoInFile(
  fileAbsPath,
  reg
): [RailsDefinitionInformation, string] {
  let definitionInformation: RailsDefinitionInformation = {
    file: null,
    line: 0,
    column: 0
...
export function findFunctionOrClassByClassNameInFile(
  ...
  var [definitionInformation, classDeclaration] = getFunctionOrClassInfoInFile(
      fileAbsPath,
      reg
    ),
    lineIndex = definitionInformation.line;
  while (-1 === lineIndex) {
    let [, symbol] = classDeclaration.split("<"),
    ...
    [definitionInformation, classDeclaration] = getFunctionOrClassInfoInFile(
      fileAbsPath2,
      reg
    );
    lineIndex = definitionInformation.line;
...
export function findFunctionOrClassByClassName(
  ...
  if (-1 !== lineIndex) {
    ...
    let beforeRange = new vscode.Range(new vscode.Position(0, 0), position),
    ...
    return new Promise<RailsDefinitionInformation>((resolve, reject) => {
      let definitionInformation = findFunctionOrClassByClassNameInFile(
        fileAbsPath,
        reg
      );
